...
- (id)initWithConnection:(NSConnection *)conn
{
  ...
  self.monitorOwner = (OSEFileSystem *)[conn rootProxy];

  self.threadDict = [[NSThread currentThread] threadDictionary];
  [self.threadDict setValue:[NSNumber numberWithBool:NO] 
...
- (oneway void)_startEventMonitorThread
{
  ...
  [self.threadDict setValue:[NSNumber numberWithBool:YES] 
		forKey:@"ThreadShouldCheckForEvents"];
}
...
//	  NSLog(@"[OSEFileSystem -checkFSEvents] event flags %@ on ident %i", flagsString, event_data[i].ident);
	  ...
	  [self.monitorOwner fileSystemEventsOccured:[self operationsFromEvents:opsList]
	        			 atPath:pathString];
	}
    }
}
