...
- download or copy sources into build directory (`BUILD_ROOT`);
- runs build and install of component;

Scripts assume that you preform run inside this directory as user. Scripts use `sudo` to install build results and ask you for the password when it's needed.
...
Note that this is "minimal" startup configuration, which uses a `xinitrc` script to start everything up (usually invoked as part of `startx`). If you want to use Login.app or start Workspace in any other way, you may have to create your own configuration.
